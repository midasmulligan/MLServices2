...
class probabilisticEWMA:
    ...
    def  simulatedUpdateAnomalyScore(self, data): 
        #Testing Mode
        dictObj = self.getMovingParameters(  )

        lastCnt = int (dictObj["simcount"])

        #get  s1 to redis s1
        s1 = float ( dictObj["sims1"] )

        #get s2 to redis  s2
        s2 = float ( dictObj["sims2"] )
        ...
        for ind in range ( 1, len( data ) ):
            #ind = vind + lastCnt
            arc_alpha = 1 - (1.0 / (ind + lastCnt) ) 
              
            arcX_t = data [ind - 1]
            X_t = data [ind]

            Z_t = (X_t - arcX_t) / arc_alpha
            ...
            s1 = (alpha * s1) + ((1 - alpha) * X_t)
            s2 = (alpha * s2) + ((1 - alpha) * math.pow ( X_t, 2 ))
                         
            arcX_t = s1
            arc_alpha = math.sqrt ( s2 - math.pow ( s1, 2 ) )

        #save sim1 to redis s1
        #save sim2 to redis  s2

        count = lastCnt + len( data )

        self.setMovingParameters( sims1=s1, sims2=s2, simcount=count )    
    ...
    def simulatedPredict (self, data):
        '''
            obtain index
        '''
        problist = self.simulatedUpdateAnomalyScore( data )
        out = [i for i,x in enumerate (problist) if x < 0.00000044]
        return out
...
if __name__ == "__main__":
    ...
    print pbObj.simulatedPredict ( data )
    print pbObj.simulatedPredict ( data )
